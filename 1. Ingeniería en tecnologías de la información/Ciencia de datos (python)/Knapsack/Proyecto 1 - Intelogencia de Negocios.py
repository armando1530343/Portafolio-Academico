...
class Knapsack:
	...
	def AG(self, num_genetico, semilla):
		...
		self.fitness_evaluation(self.poblacion, self.num_individuos)
		...
		return self._aptitud, promedio, desviacion, promedios
	...
	def fitness_evaluation(self, _poblacion, num_mejores):
		...
		for i in range(num_mejores):
			best_aptitudes.append(aptitudes[i])
			best_individuos.append(_poblacion[i])
	...
	def supervivencia(self, num):
		...
		self.poblacion = self.nvapoblacion
...
for p in range(1):
	...
	if p > 11: kp = 50
	file = ''
	if p+1 <= 9: file = 'KP'+ str(kp) + '_0' + str(p+1)
	else: file = 'KP'+ str(kp) + '_' + str(p+1)
	file = 'KP50_12'
	...
	aptitud = []
	for i in range(1):
		apt, promedio, desviacion, prom = datos.AG(1,i)
		ag1 = [['Aptitud: ' + str(apt)],['Promedio: ' + str(promedio)], ['Desviacion: ' + str(desviacion)]]

		apt2, promedio2, desviacion2, prom2 = datos.AG(2,i)
		...
		if file == 'KP50_12' and i == 0:
			...
			p1=plt.plot(apt,label="AG1 (Aptitud)")
			p2=plt.plot(prom,label="AG1 (Promedio)")
			p3=plt.plot(apt2,label="AG2 (Aptitud)")
