...
class Knapsack:
	...
	def read(self, instancia):
		...
		self.filename = instancia
		#nombre del archivo
		text = 'Knapsack_Problem_Instances/' + self.filename + '.txt'
	...
	def AG(self, num_genetico, semilla):
		...
		#print(np.array(self.poblacion))
		basura, basura2, best_aptitud, basura3, best_promedio = self.fitness_evaluation(self.poblacion, self.num_individuos)

		f1 = open('Resultados/' + self.filename + '_PROMEDIOS_CONVERGENCIA_AG' + str(num_genetico) + '.txt', 'w')
		f2 = open('Resultados/' + self.filename + '_APTITUDES_CONVERGENCIA_AG' + str(num_genetico) + '.txt', 'w')
		f3 = open('Resultados/' + self.filename + '_SOLUCIONES.txt', 'w')
		f4 = open('Resultados/' + self.filename + '_MEJOR-SOLUCION.txt', 'w')

		for i in range(100):
			self.poblacion_hijos = self.mating_selection(self.poblacion)
			...
			a = 0
			while a < self.num_individuos-1:
				self.poblacion_hijos[a], self.poblacion_hijos[a+1] = self.operador_cruza(self.poblacion_hijos[a], self.poblacion_hijos[a+1],1)
				a+=2


			#print("\nPOBLACION DE HIJOS (CRUZA)\n", np.array(self.poblacion_hijos))
			
			#print("PADRES:\n",np.array(auxPoblacion1))
			self.poblacion_hijos = self.mutacion(self.poblacion_hijos)
			#print('\nPOBLACION DE PADRES\n', np.array(auxPoblacion1))
			#print('\nPOBLACION DE HIJOS\n', np.array(self.poblacion_hijos))
			self.poblacion = [x[:] for x in auxPoblacion1]
			#print('\nMUTACION\n',np.array(self.poblacion_hijos))
			
			#print("HIJOS:\n",np.array(self.poblacion_hijos))
			self.poblacion.extend(self.poblacion_hijos)
			self.poblacion, solucion, aptitud, promedio = self.supervivencia(self.poblacion, 1)
			#print('\nNUEVA POBLACION DE PADRES\n', np.array(self.poblacion))
			#print('\nMEJOR SOLUCION: ', solucion, 'APTITUD: ',aptitud)
			#print('\nPROMEDIO: ', np.array(promedio))
			#print('\nAPTITUD: ', np.array(aptitud))
			f1.write(str(int(aptitud)) + '\n')
			f2.write(str(int(promedio)) + '\n')
			f3.write(str(solucion) + '\n')
			
			#a = input()

			aptitudes.append(aptitud)
			promedios_aptitudes.append(promedio)


		best_individuos, best_solucion, best_aptitud, basura1, basura2 = self.fitness_evaluation(self.poblacion, 1)
		
		f4.write('SOLUCION: ' + str(best_solucion) + '\nAPTITUD: ' + str(best_aptitud))
		...
		return best_solucion, best_aptitud, aptitudes, promedios_aptitudes
	...
	def mating_selection(self, _poblacion):
		...
		#a = input()
		#auxPoblacion = [x[:] for x in _poblacion]
		i = 0
		while i < self.num_individuos-1:
			poblacion_hijos[i] = _poblacion[variacion[i+1]]
			poblacion_hijos[i+1] = _poblacion[variacion[i]]
			i+=2

		#poblacion_hijos = np.array(poblacion_hijos)


		#print('\nPOBLACION DE HIJOS (APAREAMIENTO/VARIACION)\n', np.array(poblacion_hijos))
	...
	def mutacion(self, _poblacion):
		i = 0
		for hijos in _poblacion:
	...
	def fitness_evaluation(self, _poblacion, num_mejores):
		...
		while(len(aux_aptitudes) < len(aptitudes)):
			...
			aptitudes[aptitudes.index(x)] = -1

		#print('\nAPTITUDES DE POBLACION\n',aux_aptitudes)

		for i in range(num_mejores):
			if i == 0: 
				best_aptitud = aux_aptitudes[i]
				best_solucion = _poblacion[indices[i]]
			best_aptitudes.append(aux_aptitudes[i])
			best_individuos.append(_poblacion[indices[i]])

		promedio_aptitudes = sum(best_aptitudes)/len(best_aptitudes)
		#print('\nPROMEDIO: ', np.array(promedio_aptitudes))
		#print('\nAPTITUD: ', np.array(best_aptitud))
		...
		return best_individuos, best_solucion, best_aptitud, best_aptitudes, promedio_aptitudes

	def supervivencia(self, union, num):
		#SELECCION PARA SUPERVIVENCIA----------------------------------------------------

		union_poblacion = []
		mejor_mitad = []
		#print("\nUNION\n", np.array(union))
		mejor_mitad, best_solucion, best_aptitud, basura1, promedio_aptitudes = self.fitness_evaluation(union, self.num_individuos)
		#print("\nNUEVA POBLACION DE PADRES\n", np.array(mejor_mitad))
		return mejor_mitad, best_solucion, best_aptitud, promedio_aptitudes
...
f1 = open('Resultados/ESTADISTICAS_RND.txt','w')
f2 = open('Resultados/ESTADISTICAS_GR1.txt','w')
f3 = open('Resultados/ESTADISTICAS_GR2.txt','w')
f4 = open('Resultados/ESTADISTICAS_GR3.txt','w')
f5 = open('Resultados/ESTADISTICAS_AG1.txt','w')
f6 = open('Resultados/ESTADISTICAS_AG2.txt','w')
...
for p in range(1):
	...
	filename = file + '  CAPACIDAD: ' + str(capacidad)
	...
	for i in range(3):
		aptitud = datos.RND()
		best_individuos, best_aptitud, best_aptitudes, promedio_aptitudes = datos.AG(1,i)
		best_individuos2, best_aptitud2, best_aptitudes2, promedio_aptitudes2 = datos.AG(2,i)
		
		aptitud_rnd.append(aptitud)
		aptitud_ag1.append(best_aptitud)
		aptitud_ag2.append(best_aptitud2)
	...
	promedio_rnd = round(sum(aptitud_rnd)/len(aptitud_rnd), 2)
	for a in aptitud_rnd:_rnd.append((a - promedio_rnd)**2)
	desviacion_rnd = round(math.sqrt(sum(_rnd)/len(_rnd)), 2)

	promedio_ag1 = round(sum(aptitud_ag1)/len(aptitud_ag1), 2)
	for a in aptitud_ag1:_ag1.append((a - promedio_ag1)**2)
	desviacion_ag1 = round(math.sqrt(sum(_ag1)/len(_ag1)), 2)

	promedio_ag2 = round(sum(aptitud_ag2)/len(aptitud_ag2), 2)
	for a in aptitud_ag2:_ag2.append((a - promedio_ag2)**2)
	desviacion_ag2 = round(math.sqrt(sum(_ag2)/len(_ag2)), 2)

	print(aptitud_rnd)
	print(sum(aptitud_rnd)/len(aptitud_rnd))

	ag1 = [['Aptitud: ' + str(best_aptitud)],['Promedio: ' + str(promedio_ag1)], ['Desviacion: ' + str(desviacion_ag1)]]
	ag2 = [['Aptitud: ' + str(best_aptitud2)],['Promedio: ' + str(promedio_ag2)], ['Desviacion: ' + str(desviacion_ag2)]]

	f1.write(str(int(promedio_rnd)) + '\n')
	f2.write(str(int(gr1)) + '\n')
	f3.write(str(int(gr2)) + '\n')
	f4.write(str(int(gr3)) + '\n')
	f5.write(str(int(promedio_ag1)) + '\n')
	f6.write(str(int(promedio_ag2)) + '\n')
						

	conjunto_datos.append([filename, ['Aptitud: ' + str(aptitud), 'Promedio: ' + str(promedio_rnd), 
								      'Desviacion: ' + str(desviacion_rnd)], gr1, gr2, gr3, ag1, ag2])
