...
struct prestamos{
    long int isbn; int id, mat, dia, diaE, mes, mesE, retraso, tiempo, multa, entregado;
...
void librosRepetidos(struct nuevosLibros **libro, int numLibros){
    ...
    for(i=0;i<numLibros;i++){
        ...
        //fprintf(libRep,"[ISBN:%d Libros:%d] \n",repetido[i].isbn,rep);
...
int prestamoLibros(struct prestamos **presta, struct nuevosLibros **libro, struct nuevosAlumnos **alumno,
    ...
    //for(i=0;i<numAlumnos;i++){fread(&((*alumno)),sizeof(struct nuevosAlumnos),1,alum);}
    printf("\nSISTEMA DE PRESTAMOS DE LIBROS\n\n  PRESTAR LIBROS\n");
            ...
            for(i=0;i<c;i++){
                ...
                *presta=(struct prestamos*)realloc(*presta,sizeof(struct prestamos)*(numPrestamos+(i+1)));
                ...
                for(j=n;j<=f;j++){
                    ...
                    if(compara!=0 && compara2!=0){
                        compara=strcmp(fecha[j].day,"JUEVES");
                        compara2=strcmp(fecha[j].day,"VIERNES");
                        if(compara==0||compara2==0){
                            w=4;
                        }
                        if(j>(n+w)){
                ...
                //(*presta)[numPrestamos].retraso=retraso;
                //(*presta)[numPrestamos].multa=multa;

                //(*presta)[numPrestamos+i].retraso=retraso;
                if((*alumno)[x].modo=='A'){
                    printf("\n  Nombre del Alumno: %s %s %s %s",(*presta)[numPrestamos+i].n1,(*presta)[numPrestamos+i].n2,(*presta)[numPrestamos+i].a1,(*presta)[numPrestamos+i].a2);
                }
                else{
