...
int main(){
    ...
    int opc,numLibros=0,numAlumnos=0,numRep=0;
    ...
    do{
        ...
        switch(opc){
            ...
            case 1://RECIBIR LIBROS
                ...
                numLibros=recibeLibros(&libro,nL,numLibros);
                librosRepetidos(&libro,numLibros,numRep,nR);
                mostrarLibros(&libro,numLibros);
                system("pause");
                break;
            case 2://MIS ALUMNOS
...
void librosRepetidos(struct nuevosLibros **libro, int numLibros, int numRep, FILE *nR){
    system("cls");
    printf("\nSISTEMA DE PRESTAMOS DE LIBROS\n\n");
    printf("\n  LIBROS REPETIDOS \n");
    FILE *bib=fopen("MiBiblioteca.txt","rb");
    FILE *libRep=fopen("LibrosRepetidos.txt","wb");
    FILE *repe=fopen("repetidos.txt","wb");
    int rep=1, i, j, isbn;
    for(i=0;i<numLibros;i++){
        fread(&((*libro)[i]),sizeof((*libro)[i]),1,bib);
        //printf("\n  ISBN:%d  Libros:%d",(*libro)[i].isbn,rep);
        for(j=0;j<numLibros;j++){
            fread(&((*libro)[j]),sizeof((*libro)[j]),1,bib);
        ...
        if(i==numLibros-1){break;}
        fprintf(libRep,"[ISBN:%d Libros:%d]",(*libro)[i].isbn,rep);
        printf("\n  ISBN:%d  Libros:%d",(*libro)[i].isbn,rep);

        rep=1;
    }
    printf("\n\n");
    //nR=fopen("CantidadRepetidos.txt","w");
    //fprintf(nR,"%d",numRep);
    //fclose(nR);
    fclose(bib);
    fclose(libRep);
