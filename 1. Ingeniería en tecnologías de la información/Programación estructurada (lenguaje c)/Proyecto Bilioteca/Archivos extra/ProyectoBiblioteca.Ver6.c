...
struct nuevosAlumnos{
    char n1[12],n2[12],a1[12],a2[12],nc[50];
    int mat;
};
...
void calendario(int dia, int mes){
    struct a2016{
        int dia,mes,anio;
        char day[10];
    }fecha[366];
    ...
    for(n=213;n<366;n++){
        ...
        if(n<244){fecha[i].mes = 8;finMes=31;fecha[i].dia = k;}
        if(n>=244){fecha[i].mes = 9;finMes=30;fecha[i].dia = k;}
        if(n>=274){fecha[i].mes = 10;finMes=31;fecha[i].dia = k;}
        if(n>=305){fecha[i].mes = 11;finMes=30;fecha[i].dia = k;}
        if(n>=335){fecha[i].mes = 12;finMes=31;fecha[i].dia = k;}
        //printf("\ndia:%d  mes:%d  %s",fecha[i].dia,fecha[i].mes,fecha[i].day);
        i++; j++; k++;
    }
    for(i=0;i<153;i++){
        if(dia==fecha[i].dia&&mes==fecha[i].mes){
            printf("\nFecha de hoy: %s %d %d %d",fecha[i].day,fecha[i].dia,fecha[i].mes,2016);
            break;
        }
    }
    printf("\n\n");
